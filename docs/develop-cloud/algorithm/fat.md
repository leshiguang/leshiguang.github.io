...
| 字段 | 类型 | 描述 | 备注 |
| --- | --- | --- | --- |
| visceralFat | double | 内脏脂肪指数 | 保留0位小数 |
| basalMetabolism | double | 基础代谢量 | 保留0位小数 |
| protein | double | 蛋白质，单位:% | 保留1位小数 |
| ageOffset | int | 身体年龄偏移，实际年龄+年龄偏移=身体年龄 | ​
 |
| bodyScore | double | 身体得分，0-100分 | 保留1位小数 |
| bodyStyle | String | 体型 | 取值：隐性肥胖型，偏胖型，结实偏胖型，缺乏运动型，标准型，健壮型，精瘦型，模特型，健美型 |
| age | int | 年龄 | ​
 |
| sex | int | 性别，男-1，女-2 | ​
 |
...
| pbf | double | 脂肪率， % | 保留2位小数 |
| muscle | double | 肌肉量，单位:kg | 保留2位小数 |
| beatPercent | double | 击败百分比 | 保留1位小数 |
| water | double | 水分率 | 保留2位小数 |
| bone | double | 骨骼量, 单位:kg | 保留2位小数 |
| resistance50k | double | 电阻值，同入参的resistance | ​
 |
| ffm | double | 去脂体重，kg | 体重 - 脂肪量 |
| bfm | double | 脂肪量，kg | 保留1位小数 |
| rateOfMuscle | double | 肌肉率 ，% | 保留1位小数 |
| skeletonMuscle | double | 骨骼肌，kg | 保留1位小数 |
| bodyAge | int | 身体年龄 | ​
 |
| visceralFatLevel | int | 内脏脂肪等级，0-理想，1-偏高，2-较高，3-超高 |  |
| fatControl | double | 脂肪控制，小于 0 表示建议增加脂肪量， 大于 0 表示应减少脂肪量 | 保留1位小数 |
