...
- 功能：互联秤在绑定过程中，需要动态写入deviceId，写入时机在收到onDeviceOperationCommandUpdate指令之后
...
- 功能：在设备配对或绑定过程中，输入操作指令，写入时机在收到onDeviceOperationCommandUpdate指令之后
...
参数：<br />① int status,配网结果 0：成功，1:失败<br />② String ssid,配网成功的Wi-Fi名称
<a name="xG4mD"></a>
## OnDeviceUpgradeListener
...
<a name="4xZ2G"></a>
### PedometerHeartRateStatisticsData（心率状态数据）
...
<a name="0eVMp"></a>
### PedometerRunningCalorieData（运动卡路里数据）
...
String ssid Wi-Fi名称<br />byte[] bssid Wi-Fi的bssid<br />int authMode Wi-Fi安全类型<br />int rssi Wi-Fi信号强度<br />int status Wi-Fi状态
<a name="qjb80"></a>
# 算法分析
<a name="oIq11"></a>
## 体重算法
